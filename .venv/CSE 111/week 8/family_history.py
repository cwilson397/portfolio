...
def main():
    ...
    # calls the count marriages function
    count_marriages(marriages_dict, people_dict)


def print_death_age(people_dict):
    for person in people_dict.values():
        name, birth_year, death_year = person[NAME_INDEX], person[BIRTH_YEAR_INDEX], person[DEATH_YEAR_INDEX]
        age_at_death = death_year - birth_year
        print(f"{name} was born in {birth_year} and died in {death_year} at age {age_at_death}.")


def count_genders(people_dict):
    male_count = sum(1 for person in people_dict.values() if person[GENDER_INDEX] == "M")
    female_count = sum(1 for person in people_dict.values() if person[GENDER_INDEX] == "F")
...
def print_marriages(marriages_dict, people_dict):
    for marriage in marriages_dict.values():
        husband, wife, wedding_year = marriage[HUSBAND_KEY_INDEX], marriage[WIFE_KEY_INDEX], marriage[WEDDING_YEAR_INDEX]
        husband_name, husband_birth = people_dict[husband][NAME_INDEX], people_dict[husband][BIRTH_YEAR_INDEX]
        wife_name, wife_birth = people_dict[wife][NAME_INDEX], people_dict[wife][BIRTH_YEAR_INDEX]
        print(f"{husband_name} married {wife_name} in {wedding_year}.")
        print(f"  - {husband_name} was {wedding_year - husband_birth} years old.")
        print(f"  - {wife_name} was {wedding_year - wife_birth} years old.")

def count_marriages(marriages_dict, people_dict):
    marriage_counts = {}
    for marriage in marriages_dict.values():
        husband, wife = marriage[HUSBAND_KEY_INDEX], marriage[WIFE_KEY_INDEX]
        marriage_counts[husband] = marriage_counts.get(husband, 0) + 1
        marriage_counts[wife] = marriage_counts.get(wife, 0) + 1
    
    most_married = max(marriage_counts, key=marriage_counts.get)
    print("Number of marriages per person:")
    for person, count in marriage_counts.items():
        print(f"{people_dict[person][NAME_INDEX]}: {count}")
    print(f"Most marriages: {people_dict[most_married][NAME_INDEX]} with {marriage_counts[most_married]} marriages.")
