    ...
    try:
        ...
        with open(filename, "rt") as text_file:
            ...
            try:
                ...
                print(f"Error: Invalid input for percentage. {e}")
                return
            ...
            for line_num, row in enumerate(reader, start=2):  # Start at 2 to account for the header line
                try:
                    ...
                    print(f"Error: Division by zero in file '{filename}' at line {line_num}. 'Fatal Crashes' column contains zero.")
                except ValueError as e:
                    print(f"Error: Invalid data format in file '{filename}' at line {line_num}. {e}")
    except FileNotFoundError:
        print("Error: The file was not found. Please check the filename and try again.")
    except csv.Error as e:
        print(f"Error: Problem reading the CSV file '{filename}'. {e}")
...
def estimate_reduction(row, behavior_key, perc_reduc):
    ...
    if fatal_crashes == 0:
        raise ZeroDivisionError("Fatal crashes column contains zero, cannot divide.")
